...
public final class AdminItemInspectorMenu {
    ...
    public void open(final Player player, final AdminCatalogViewState state, final String itemKey, final String returnBucketId, final String returnRuleId, final int pageIndex, final String sortMode) {
    ...
    public void handleClick(final InventoryClickEvent event, final AdminMenuHolder holder) {
        ...
        switch (event.getRawSlot()) {
            case 38 -> { if (holder.returnBucketId() != null) this.adminMenuRouter.openReviewBucketDetail(player, holder.state(), holder.returnBucketId(), holder.pageIndex(), holder.sortMode()); }
            case 40 -> { if (holder.returnRuleId() != null) this.adminMenuRouter.openRuleImpactDetail(player, holder.state(), holder.returnRuleId(), holder.pageIndex(), holder.sortMode()); }
            case 42 -> {
    ...
    private ItemStack decisionItem(final AdminCatalogDecisionTrace trace) {
        final List<String> lore = new ArrayList<>();
        if (trace.classifiedCategory() == trace.finalCategory()) lore.add("Category: " + trace.finalCategory().name());
        else lore.add("Category: " + trace.classifiedCategory().name() + " -> " + trace.finalCategory().name());
        ...
        lore.add("Final policy: " + trace.finalPolicy().name());
        if (trace.postRuleAdjustment() != null && !trace.postRuleAdjustment().isBlank()) lore.add("Adjustment: " + trace.postRuleAdjustment());
        return this.item(Material.BOOK, "Decision", lore);
    }

    private ItemStack runtimeItem(final AdminCatalogPlanEntry planEntry) {
        final List<String> lore = new ArrayList<>();
        if (planEntry == null) lore.add("No runtime plan entry recorded.");
        else {
    ...
    private ItemStack ruleItem(final AdminCatalogDecisionTrace trace, final AdminCatalogViewState state) {
        final List<String> lore = new ArrayList<>();
        if (trace.winningRuleId() == null || trace.winningRuleId().isBlank()) lore.add("Decision source: none recorded.");
        else if (trace.matchedRuleIds().contains(trace.winningRuleId())) lore.add("Decision source: matched rule " + trace.winningRuleId());
        else lore.add("Decision source: fallback rule " + trace.winningRuleId());
        ...
        for (final String matchedRuleId : trace.matchedRuleIds()) if (!matchedRuleId.equals(trace.winningRuleId())) matchedButLost.add(matchedRuleId);
        if (!matchedButLost.isEmpty()) lore.add("Matched but lost: " + matchedButLost);
    ...
    private ItemStack reviewBucketItem(final AdminCatalogDecisionTrace trace, final AdminCatalogPlanEntry planEntry) {
        ...
        if (bucketIds.isEmpty()) lore.add("No current review bucket membership.");
        else for (final String bucketId : bucketIds) lore.add(bucketId);
    ...
    private ItemStack actionHintItem(final AdminCatalogDecisionTrace trace, final AdminCatalogViewState state) {
        ...
        if (!reviewBuckets.isEmpty()) lore.add("Bucket focus: " + reviewBuckets.get(0));
        if (trace.finalCategory() == CatalogCategory.MISC && trace.finalPolicy() != CatalogPolicy.DISABLED) lore.add("Likely next action: category review.");
        else if (trace.derivationReason() == DerivationReason.NO_RECIPE_AND_NO_ROOT) lore.add("Likely next action: root anchor or leave disabled.");
        else if (trace.derivationReason() == DerivationReason.ALL_PATHS_BLOCKED) lore.add("Likely next action: recipe-path review.");
        else if (trace.finalPolicy() == CatalogPolicy.SELL_ONLY) lore.add("Likely next action: review long-term SELL_ONLY fit.");
        else lore.add("Likely next action: none.");
        if (trace.winningRuleId() != null && !trace.winningRuleId().isBlank()) lore.add("Rule shortcut available below.");
    ...
    private List<String> findReviewBuckets(final AdminCatalogDecisionTrace trace, final AdminCatalogPlanEntry planEntry) {
        ...
        if (planEntry != null && planEntry.policy() != CatalogPolicy.DISABLED && planEntry.category() == CatalogCategory.MISC) buckets.add("live-misc-items");
        if (trace.derivationReason() == DerivationReason.NO_RECIPE_AND_NO_ROOT) buckets.add("no-root-path");
        if (trace.derivationReason() == DerivationReason.ALL_PATHS_BLOCKED) buckets.add("blocked-paths");
        if (trace.manualOverrideApplied()) buckets.add("manual-overrides");
        if (trace.finalPolicy() == CatalogPolicy.SELL_ONLY) buckets.add("sell-only-review");
        return buckets;
    }

    private ItemStack contextButton(final String id, final String label) {
        if (id == null || id.isBlank()) return this.item(Material.GRAY_STAINED_GLASS_PANE, label, List.of("Unavailable"));
    ...
    private ItemStack winningRuleButton(final AdminCatalogDecisionTrace trace, final AdminCatalogViewState state) {
        if (trace.winningRuleId() == null || trace.winningRuleId().isBlank()) return this.item(Material.GRAY_STAINED_GLASS_PANE, "Winning rule", List.of("Unavailable"));
