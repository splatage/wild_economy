...
public final class ConfigLoader {
    ...
    public TitleSettingsConfig loadTitleSettingsConfig() {
        ...
        for (final String titleKey : titlesSection.getKeys(false)) {
            ...
            final TitleSource source = this.parseTitleSource(
                    this.requireNonBlank(section, "source", "title option '" + titleKey + "'"),
                    titleKey
            );
            final TitleSection titleSection = this.parseTitleSection(this.getOptionalString(section, "section"), titleKey, source);
    ...
    private TitleSection parseTitleSection(final String rawValue, final String titleKey, final TitleSource source) {
        if (rawValue == null || rawValue.isBlank()) {
            return switch (source) {
                case RELIC -> TitleSection.RELIC;
                case COMMERCE_MILESTONE, COMMERCE_CROWN -> TitleSection.BEST_OF_ALL_TIME;
                case ACHIEVEMENT, EVENT -> TitleSection.ACHIEVEMENT;
                case AUTHORITY, SUPPORTER -> TitleSection.AUTHORITY;
                case TIME_ON_SERVER -> TitleSection.TIME_ON_SERVER;
            };
        }
        try {
            return TitleSection.fromConfig(rawValue);
        } catch (final IllegalArgumentException exception) {
            throw new IllegalStateException(
                    "title-settings.yml title option '" + titleKey + "' has invalid section '" + rawValue + "'",
                    exception
            );
        }
    }
