# wild_economy Phase 2 pass 1 (`2c65df280edd13f97f9756330c0eb236e0c4e39e`)

This bundle contains complete replacement files for the first backend-focused Phase 2 admin pass.

Scope of this pass:
- add `/shopadmin item <item_key>` inspector support
- generate backend rule-impact reporting
- generate grouped review-bucket reporting for unresolved items, blocked paths, live MISC spillover, manual overrides, and SELL_ONLY review
...
public final class AdminCatalogPhaseOneService {
    ...
    private List<AdminCatalogRuleImpact> buildRuleImpacts(
        ...
        for (final AdminCatalogPolicyRule rule : rules) {
            ...
            final Map<CatalogPolicy, Integer> policyCounts = new EnumMap<>(CatalogPolicy.class);
            for (final CatalogPolicy policy : CatalogPolicy.values()) {
                policyCounts.put(policy, Integer.valueOf(0));
            }
            winningPolicies.put(rule.id(), policyCounts);
        }
        ...
        for (final AdminCatalogPolicyRule rule : rules) {
            impacts.add(
                new AdminCatalogRuleImpact(
                    ...
                    rule.hasMatchCriteria(),
                    ruleMatchCounts.getOrDefault(rule.id(), Integer.valueOf(0)).intValue(),
                    ruleApplicationCounts.getOrDefault(rule.id(), Integer.valueOf(0)).intValue(),
    ...
    private List<AdminCatalogReviewBucket> buildReviewBuckets(
        ...
        for (final AdminCatalogPlanEntry entry : proposedEntries) {
            if (entry.policy() != CatalogPolicy.DISABLED && entry.category() == CatalogCategory.MISC) {
                buckets.get("live-misc-items").add(entry.itemKey());
            }
            if (entry.policy() == CatalogPolicy.SELL_ONLY) {
                buckets.get("sell-only-review").add(entry.itemKey());
            }
        }

        for (final AdminCatalogDecisionTrace trace : decisionTraces) {
            if (trace.derivationReason() == DerivationReason.NO_RECIPE_AND_NO_ROOT) {
                buckets.get("no-root-path").add(trace.itemKey());
            }
            if (trace.derivationReason() == DerivationReason.ALL_PATHS_BLOCKED) {
                buckets.get("blocked-paths").add(trace.itemKey());
            }
            if (trace.manualOverrideApplied()) {
                buckets.get("manual-overrides").add(trace.itemKey());
            }
        }
    ...
    private static final class AdminCatalogReviewBucketBuilder {
        ...
        private void add(final String itemKey) {
            this.count++;
        ...
        private AdminCatalogReviewBucket build() {
            return new AdminCatalogReviewBucket(this.bucketId, this.description, this.count, this.sampleItems);
        }
    }
...
## File: `src/main/java/com/splatage/wild_economy/catalog/admin/AdminCatalogRuleImpact.java`
...
## File: `src/main/java/com/splatage/wild_economy/catalog/admin/AdminCatalogReviewBucket.java`
